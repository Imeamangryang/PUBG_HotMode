...
- World Item 표시: 단일 generic Blueprint + DataTable `UStaticMesh`
...
| 영역            | 현재 상태                                                                       | 변경 방향                                         |
| --------------- | ------------------------------------------------------------------------------- | ------------------------------------------------- |
| Health          | `ABG_PlayerState`의 `CurrentHP`, `MaxHP`, `bIsDead`, `ApplyDamage`              | `UBG_HealthComponent`로 이동                      |
| Damage          | `UBG_DamageSystem`이 target PlayerState에 damage 적용                           | target Character의 HealthComponent에 damage 적용  |
| Character state | `CharacterState`, `bIsDead`, `bIsWeaponEquipped`, `EquippedWeaponPoseType` 복제 | Equipment adapter로 동기화                        |
| Health UI       | `UBG_HealthViewModel`이 PlayerState에 바인딩                                    | possessed Character의 HealthComponent에 바인딩    |
...
- item별 World Item actor class override
- 최종 밸런스 데이터 확정
...
- GameplayTags config: `Config/DefaultGameplayTags.ini`
- AssetManager config: item registry Primary Asset Type
...
| `ABG_WorldItemActor`            | 서버 소유 pickup/drop world item 상태                                 |
...
- World Item 표시: `UStaticMesh`, icon, display data
...
- Actor: `ABG_WorldItemActor`
- Blueprint: 단일 generic BP
- 표시 mesh: `UStaticMesh`
- 표시 데이터: DataTable row
- category별 child BP 없음
- SkeletalMesh 없음
- actor class override 없음
...
│   ├── BG_WorldItemActor.h
│   └── BG_WorldItemActor.cpp
├── Health/
│   ├── BG_HealthComponent.h
│   └── BG_HealthComponent.cpp
├── UI/
